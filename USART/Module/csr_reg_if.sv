...
module csr_reg_if(
    ...
    /* TX data: forward pwdata directly during write for zero-latency FIFO push */
    assign tx_data  = (psel && penable && pwrite && (reg_sel == 4'h2))
                      ? pwdata[7:0] : reg_txdata[7:0];
    assign tx_write = (psel && penable && pwrite && (reg_sel == 4'h2));
    ...
    always @(posedge pclk or negedge presetn) begin
        if (!presetn) begin
            ...
            case (reg_sel)
                4'h0: reg_ctrl   <= pwdata;
                4'h1: reg_baud   <= pwdata;
                4'h2: reg_txdata <= pwdata;
                default: ;
            endcase
        end
    end

    /* RX data latch and status */
    always @(posedge pclk or negedge presetn) begin
        if (!presetn) begin
            reg_rxdata <= 32'd0;
            reg_status <= 32'd0;
            ...
            /* TX FIFO flags - level signals, updated every clock */
            reg_status[3]  <= tx_fifo_full;
            reg_status[4]  <= tx_fifo_empty;
            reg_status[5]  <= tx_fifo_ovf;
            reg_status[6]  <= tx_fifo_udf;
            /* RX FIFO flags - level signals, updated every clock */
            reg_status[7]  <= rx_fifo_full;
            reg_status[8]  <= rx_fifo_empty;
            reg_status[9]  <= rx_fifo_ovf;
            reg_status[10] <= rx_fifo_udf;
            /* clear sticky error bits on read */
            if (psel && penable && !pwrite && (reg_sel == 4'h4))
                reg_status[2:0] <= 3'd0;
        end
    end
 
    /* APB read mux - purely combinatorial.
     * prdata is valid whenever psel && !pwrite, no clock needed. */
    always @(*) begin
